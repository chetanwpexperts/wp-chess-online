...
preload.src="./chessset/h_white.gif";
preload.src="./chessset/h_black.gif";
for (index=0; index<10; index++)
	preload.src="./chessset/d"+index+".gif";
...
function gotoMove(move_id)
{
	...
	if (move_id%2==0)
		document.images["colorpin"].src="./chessset/h_white.gif";
	else
		document.images["colorpin"].src="./chessset/h_black.gif";
...
function renderMoveNumber(round)
{
	...
	document.images["digit1"].src="./chessset/d"+d1+".gif";
	document.images["digit2"].src="./chessset/d"+d2+".gif";
}
...
/* Show difference in chessmen using image slots 'tslot0-15' (below move history). */
function showDiff()
{
	//for (i=0; i<15; i++)
		//document.images["tslot"+i].src="./chessset/sempty.gif";
		...
		if (bottom=="b" && diff[4-i]<0) {
			for (j=0; j<-diff[4-i]; j++) {
				document.images["tslot"+slot_id].src=
					"./chessset/sb"+name+".gif";
				slot_id++;
			}
		} else if (bottom=="w" && diff[4-i]>0) {
			for (j=0; j<diff[4-i]; j++) {
				document.images["tslot"+slot_id].src=
					"./chessset/sw"+name+".gif";
				slot_id++;
			}
		}
	}
	...
	if (slot_id > 0) {
		document.images["tslot"+slot_id].src="./chessset/sempty.gif";
		slot_id++;
	}
	/* Top player next */
	for(i=0; i<5; i++) {
...
function renderBoard()
{
	for (i=0; i<64; i++) {
		if (board[i]==0) {
			document.images["b"+i].src="./chessset/empty.gif";
			continue;
		}
		value=board[i];
		if (value >=7) {
		...
		document.images["b"+i].src="./chessset/"+pref+chessman;
	}
	showDiff();   
}
</script>
