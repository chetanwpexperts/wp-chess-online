...
function autocomplete($pcolor, $move)
{
    ...
    * process the same. */
    ...
    if ($pawn_upg != 'N' && $pawn_upg != 'B' && $pawn_upg != 'R' && $pawn_upg != 'Q' &&
        $pawn_upg != '?') {
    ...
    if ($move[0] >= 'a' && $move[0] <= 'h') {
        ...
        if (strlen($move) == 4) {
            ...
            if ($pcolor == 'w')
                ...
                $src_y = $dest_y + 1;
            return sprintf('P%s%dx%s%d%s', $src_x, $src_y, $dest_x, $dest_y, $pawn_upg);
        } else
            if (strlen($move) == 2) {
                ...
                if ($move[1] >= '1' && $move[1] <= '8') {
                    ...
                    * possible if single pawn in column */
                    ...
                    if ($pcolor == 'w')
                        ...
                        $dest_line = $pawn_line - 1;
                    return sprintf('P%s%dx%s%d', $move[0], $pawn_line, $move[1], $dest_line);
                }
            }
    ...
    if (!$fig1_can_reach && !$fig2_can_reach) {
        $acerror = sprintf('no %s can reach %s', getCMName($move[0]), $dest_coord);
        return null;
    }
    if ($fig1_can_reach && $fig2_can_reach) {
        /* Ambiguity - check whether a hint is given */
        if (($action == '-' && strlen($move) == 4) || ($action == 'x' && strlen($move) ==
            5))
            $hint = $move[1];
        if (empty($hint)) {
            $acerror = sprintf('both %s can reach %s', getCMName($move[0]), $dest_coord);
            return null;
        }
        if ($hint >= '1' && $hint <= '8') {
        ...
        if ($move_fig1)
            ...
            return sprintf('%s%s%s%s', $move[0], $pos2, $action, $dest_coord);
    } else {
        if ($fig1_can_reach)
            return sprintf('%s%s%s%s', $move[0], $pos1, $action, $dest_coord);
        else
...
function handleMove($gid, $uid, $move)
{
                        ...
                        if ($move == '0-0-0') {
                            ...
                            * which would result in 'no one can reach' error) */
                            if (strlen($history_move) >= 6)
                                $history_move = getHistoryNotation($cur_player, $history_move);
